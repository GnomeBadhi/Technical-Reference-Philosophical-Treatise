# fused_glyph.py
"""
Fused SKL Glyph (Dual‑Universe)

Visualizes:
- Universe A glyph
- Universe B glyph
- fused ΔC_global geometry
- dual‑phase halo
- invariant‑safe fusion
...
# Draw fused glyph
# -------------------------------------------------------------
def draw_fused_glyph(ax, dataA, dataB):
    ...
    for angle, d, A1, A2 in zip(angles, dC, A_A, A_B):
        # Fused awareness for linewidth
        A_fused = 0.5 * (A1 + A2)

        # ΔC determines spoke direction and length
...
# Public API
# -------------------------------------------------------------
def fused_glyph(pathA: str, pathB: str, frame: int = -1):
    """
    Render a fused SKL glyph for a single synchronized frame.
    """
...
if __name__ == "__main__":
    fused_glyph("universe_A.jsonl", "universe_B.jsonl")
